...
export class PostServiceService {
  ...
  likePost(postId: number){
    console.log("in like" + postId)
    return this._httpService.get("http://localhost:8080/Rev_SpringMVC_Hello/api/posts/mylike/"+postId);
  }

  sendLike(idnumber: number){
    console.log("likes");
    this._httpService.get("http://localhost:8080/Rev_SpringMVC_Hello/api/posts/mylike/"+idnumber);
  }
